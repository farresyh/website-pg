...
| Reports (RPT-1..3) | ✅ Live — ledger-sourced profit, `paid_at`-scoped sales, reseller-aware, tabbed analytics suite, CSV/PDF. **Restructure (ADR-086) complete** 2026-09-11 — PR-1 (grouped-SQL rewrite) + PR-2 (Reseller-wallet breakdown, folded into the Affiliates tab); PR-3 (chart migration) closed without a code change, no charting library — matches the existing hand-rolled-visual house style. Gemini LLM assistant (ADR-087) not built | ADR-086, 087 |
