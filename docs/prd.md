...
— grilled and locked to a strict layout-only scope (no renaming, no field
reorganization between cards, no functionality change); every conditional
action button's condition/handler/label is unchanged, only repositioned.
**PR-3 (Reports)** still open — re-open the artifact directly in a
browser before building it (the Artifact tool's own read/read_db paths
return only an empty shell for this artifact type). The other ~13 admin
sections stay out of scope until the
founder's own mockups exist for them (ADR-104 decision 7).
