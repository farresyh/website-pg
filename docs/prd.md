...
- **`CheckoutController` is now done** (2026-07-25, see its own §14 row) — `POST /api/checkout` works end-to-end against real `Package` data: resolves Game/Package/Reseller, enforces `Game.validation_rules` (built the same session, and deliberately placed in `/middleware/product-manager`'s `LinkCategoryModal`, not `/admin/games` — founder correction, see ADR-013's sibling note and the `feedback_middleware_vs_admin_config_placement` memory), and creates a real Xendit payment request. Two genuine next-phase candidates open up from here, not yet prioritized against each other:
  1. **Storefront app** (§6.16) — still **no Next.js app scaffolded at all**. The checkout API has a real, tested consumer waiting; nothing calls it from a browser yet. This is the bigger of the two (greenfield app).
  2. **Orders admin review screen** (ORD-1..7) — orders can now be created and fulfilled end-to-end, but there is still no `/admin/orders` screen to see them, filter stuck/failed ones, or trigger the existing retry-delivery/voucher resolution actions. Smaller than the Storefront, and unlocks wiring Voucher Path B's existing backend endpoint (`POST /orders/{order}/voucher`) to a real "Create Voucher" button.
- Bulk-curate more of the 316-item Gamevion catalog into real `Package`s as needed (`/middleware/product-manager`) — admin busywork, not a code blocker, for either of the above.
...
| Orders Management (ORD-1..11) | ✅ Model + fulfillment service + `CheckoutController` (`POST /api/checkout`) all done, tested | ⬜ Not started | Checkout is a real, working API endpoint now (2026-07-25) — the remaining gap is an Orders **admin review** screen (ORD-1..7's table/detail/resolve UI) and the Storefront UI to actually call this endpoint from, not backend logic |
