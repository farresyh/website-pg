...
day** — `Tag` severity remapped to the artifact's own status tokens (new
`review` severity, distinct from `warn`, for `needs_review`; opt-in `dot`
indicator), purple for every profit/markup figure, cyan for "act here"
(active filters/cards, focus ring), `page-title`/`section-title`/
`metric-lg`/`overline`/`code-id` typography tokens applied. PR-3 (Reports)
still open. The other ~13 admin sections stay out of scope until the
