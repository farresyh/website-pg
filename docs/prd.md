...
**Test suite total: 123 passing** (120 in the default sqlite suite + 3 in the MySQL concurrency suite).
...
- `Game`/`Package` models (unblocks `CheckoutController`, most of the admin catalog, Price Sync, and the Middleware Panel) are still genuinely blocked on real Gamevion catalog data. Two things to resolve with the founder before starting this: (1) confirm Gamevion is still the intended real supplier — a legacy-system walkthrough surfaced MooGold/BarbarTopup as *that* system's actual suppliers, purely as a reference data point, not a reason to change ADR-001 on its own; (2) `GAMEVION_BEARER_TOKEN`/`GAMEVION_API_KEY` are still needed to run the written-but-unexecuted Gamevion smoke test.
...
- Other not-blocked areas per §15 (Blacklist/Fraud, MFA, Backups, Settings, Image Gallery) are all fair game, in whatever order the founder wants to prioritize.
...
| Supplier Adapter (ADAPT-1..4) | 🟡 Interface + `GamevionAdapter` done | — | **Blocked**: confirm the real supplier (Gamevion vs. MooGold/BarbarTopup — see `legacy-reference-notes.md`) before continuing |
| Payment Gateway (Xendit, PAY-1..4) | 🟡 Adapter done, webhook wired | — | Reconciliation job (PAY-3) not started |
| Games & Packages (GAME-1..12) | ⬜ Model/migration don't exist | ⬜ Not started | Blocked on the supplier decision above |
| Price Sync (SYNC-1..6) | ⬜ Not started | ⬜ Not started | Blocked, same reason |
| Supplier Management (SUPP-1..5) | ✅ `Supplier` model done | ⬜ Not started | Adapter not yet wired to read from this table |
| Orders Management (ORD-1..11) | 🟡 Model + fulfillment service done, `CheckoutController` blocked | ⬜ Not started | Blocked on Games/Packages |
...
| Middleware Panel (MID-1..13, MUI-1..11) | ⬜ Not started | ⬜ Not started | Blocked — needs Games/Packages |
...
**Not a universal blocker:** the supplier decision (Gamevion vs. the legacy system's MooGold/BarbarTopup) only blocks areas that need a real product catalog — Games/Packages, Price Sync, `CheckoutController`, Orders UI, and the Middleware Panel. Withdrawals, Vouchers, Blacklist/Fraud, MFA, Backups, Settings, Image Gallery, and Dashboard/Reports scaffolding can all proceed in parallel — none of them read from the supplier catalog.
