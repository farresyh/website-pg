...
| Supplier Adapter (ADAPT-1..4) | ✅ Gamevion + Digiflazz both live. Per-supplier circuit breaker, `SupplierAdapterFactory` routing, async delivery state machine + poll backstop, inbound webhooks (HMAC). **ADR-097 PR-1 built 2026-09-16** — Digiflazz `customer_no` separator moves per-game (was wrongly supplier-wide); PR-2 (per-game Zone ID picklist, storefront + Reseller API + Bot value-validation) not yet built | ADR-006, 030–032, 067, 069, 097 |
