...
- **`/admin/orders` is now done** (2026-07-25, see its own §14 row) — a real order's outcome (from a real `POST /api/checkout` call) is now visible in the Admin Panel, list + detail, verified live. This was the last piece needed to actually *test* the checkout→payment→fulfillment→ledger pipeline end-to-end and see the result, not just trust the automated tests.
- **Storefront app** (§6.16) is now the clear single biggest remaining gap — still **no Next.js app scaffolded at all**. The checkout API has a real, tested consumer waiting (and now a way to see its results); nothing calls it from a browser yet.
