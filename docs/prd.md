...
| Reports (RPT-1..3) | ✅ Live — ledger-sourced profit, `paid_at`-scoped sales, reseller-aware, tabbed analytics suite, CSV/PDF. Restructure PR-1 (grouped-SQL rewrite) 🟡 built on `staging` 2026-09-11; PR-2 (new dimensions) + PR-3 (chart migration) + the Gemini LLM assistant (ADR-087) not built | ADR-086, 087 |
