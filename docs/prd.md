...
| **All controllers/routes/auth** (Sanctum, MFA) | ⬜ Not started — `CheckoutService`/`OrderFulfillmentService` are fully usable but nothing HTTP-facing calls them yet | — | — |
...
**Test suite total: 77 passing** (73 in the default sqlite suite + 3 in the MySQL concurrency suite).
