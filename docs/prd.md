...
- **Founder-requested first task, before any new feature work**: audit the current codebase for internal consistency and structural soundness — code, DB wiring, API wiring, and whether anything contradicts anything else across `docs/*.md`. Today's session ended with a full pass of this kind already (this note is a record of it, and a prompt to repeat the discipline periodically, not a sign it wasn't done) — see the 2026-07-25 doc-consistency-audit note below for what that pass covered and found.
- Bulk-curate more of the 316-item Gamevion catalog into real `Package`s via the now-working Product Manager screen (`/middleware/product-manager`) — this is now admin busywork, not a code blocker. Once a meaningful slice of the catalog is curated, `CheckoutController` can be built for real (not against a single manually-created test Package).
- **Per-game input fields for order submission** (new gap, surfaced 2026-07-25 — see ADR-005's addendum): Gamevion's `/api/order` takes an opaque `data` string with no schema of its own; WE must know per-game whether it needs just `player_id` or `player_id|server_id`, etc. `Game.validation_rules` (JSON) already exists in the schema for this but has no admin UI to populate it and no `CheckoutService`/`OrderFulfillmentService` logic yet to assemble the string from it. Needed before `CheckoutController` can work for any game requiring more than a bare player ID (e.g. MLBB needs a server ID).
- **Price Sync (§6.4, SYNC-1..6) — the ongoing-maintenance feature**, distinct from Stage 1 (one-off raw mirror) and Stage 2 (one-off promote): live cost-price propagation into already-promoted Packages, deactivation detection, and MYR currency conversion for non-MYR suppliers. See the scope note above (2026-07-25) — this is now a confirmed, three-part design, not just "sync more often."
...
- Admin — Games & Packages (GAME-1..5/7) **is now built and verified live** (`/admin/games` — see its own §14 row) — remaining pieces (GAME-6 drag-drop reorder, GAME-9/10 redundant with Product Manager, GAME-11 bulk price update, GAME-12/SEO fields) are lower priority, not blocking.
- Once an Orders admin page exists, wire Voucher Path B's existing backend endpoint (`POST /orders/{order}/voucher`) to a "Create Voucher" button on the order detail view — the backend and its tests are already done, this is UI-only work at that point.
- Other not-blocked areas per §15 (Blacklist/Fraud, MFA, Backups, Settings, Image Gallery) are all fair game, in whatever order the founder wants to prioritize. MFA (AUTH-7) specifically is still an open founder decision — full enforcement (login + step-up on withdrawal/voucher) vs. a lighter step-up-only version, given it adds friction; not started either way.
...
| Money core (Pricing, Ledger, Voucher, Order status/idempotency) | ✅ Full service layer, tested | — (consumed by other areas, no screen of its own) | Most mature part of the codebase |
...
| Orders Management (ORD-1..11) | 🟡 Model + fulfillment service done, `CheckoutController` blocked | ⬜ Not started | Real `Package` data now exists (at least one, more as admin curates via Product Manager) — the remaining blocker is curating enough of the catalog, not the schema or tooling |
...
| Storefront (§6.16, checkout flow) | ⬜ Not started | ⬜ **No app scaffolded yet** | Blocked — checkout needs real `Package` pricing |
