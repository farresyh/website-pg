...
**Test suite total: 131 passing** (128 in the default sqlite suite + 3 in the MySQL concurrency suite).
...
- No sync/import command exists yet to turn the live Gamevion catalog into real `Package` rows — this is the actual remaining blocker on `CheckoutController`, not the schema (SYNC-4 territory: needs the decimal-MYR→integer-sen conversion, and a decision on how admin curates/maps Gamevion's raw `category` strings onto `Game` rows — confirmed messy, e.g. `"Mobile legends Global "` vs `"Mobile Legends: Bang Bang (Malaysia)"` vs `"MLBB UNLIMITED PROMO"` all coexist in the real catalog).
...
| Games & Packages (GAME-1..12) | 🟡 Model/migration done (2026-07-25) | ⬜ Not started | No sync/import command yet — real `Package` rows don't exist |
| Price Sync (SYNC-1..6) | ⬜ Not started | ⬜ Not started | Now the actual blocker on `CheckoutController`/catalog UI — needs decimal-MYR→sen conversion + a category→Game curation decision |
| Supplier Management (SUPP-1..5) | ✅ `Supplier` model done | ⬜ Not started | Adapter not yet wired to read from this table |
| Orders Management (ORD-1..11) | 🟡 Model + fulfillment service done, `CheckoutController` blocked | ⬜ Not started | Blocked on Price Sync (real `Package` data), not on the schema anymore |
