...
| **`Order`/`Supplier` models/migrations** | ⬜ Not started — currently only pure logic services exist (`OrderStatusService`, `ReferenceNumberService`, Supplier Adapter layer, Payment Gateway layer); no Eloquent model or table yet | — | — |
...
**Test suite total: 56 passing** (54 in the default sqlite suite + 2 in the MySQL concurrency suite).
