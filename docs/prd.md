...
| Reports (RPT-1..3) | ✅ Live — ledger-sourced profit, `paid_at`-scoped sales, reseller-aware, tabbed analytics suite, CSV/PDF. Restructure PR-1 (grouped-SQL rewrite) + PR-2 (Reseller-wallet breakdown, folded into the Affiliates tab) 🟡 built on `staging` 2026-09-11; PR-3 (chart migration) + the Gemini LLM assistant (ADR-087) not built | ADR-086, 087 |
