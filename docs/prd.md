...
| **`Supplier` model/migration** | ⬜ Not started — `GamevionAdapter` still configured via env vars in `config/services.php`, not SUPP-5-compliant encrypted-at-rest DB config yet | — | — |
...
**Test suite total: 60 passing** (58 in the default sqlite suite + 2 in the MySQL concurrency suite).
