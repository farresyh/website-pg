...
/**
 * ADR-059 decision 5: the single seam for every affiliate-portal money
 * read. It must key strictly off polymorphic `owner_type='affiliate'` /
 * `owner_id` — `ledger_entries` has no `affiliate_id` scope to lean on.
 */
class AffiliateEarningsServiceTest extends TestCase
{
    use RefreshDatabase;

    private AffiliateEarningsService $earnings;
    ...
    {
        parent::setUp();
        $this->earnings = app(AffiliateEarningsService::class);
        $this->ledger = app(LedgerService::class);
    }

    private function affiliate(string $name = 'Acme Resell'): Affiliate
    {
    ...
    public function test_balance_sums_only_this_affiliates_ledger_entries(): void
    {
        $mine = $this->affiliate('Mine');
        $other = $this->affiliate('Other');

        $this->ledger->credit('affiliate', $mine->id, 5000, 'order_profit', 'order', 1);
        $this->ledger->credit('affiliate', $mine->id, -1200, 'withdrawal', 'withdrawal', 1);
        $this->ledger->credit('affiliate', $other->id, 9999, 'order_profit', 'order', 2);
    ...
    public function test_ledger_entries_returns_only_this_affiliate_newest_first_without_admin_ids(): void
    {
        $mine = $this->affiliate('Mine');
        $other = $this->affiliate('Other');

        $this->ledger->credit('affiliate', $mine->id, 1000, 'order_profit', 'order', 1, 42);
        $this->ledger->credit('affiliate', $mine->id, -300, 'affiliate_tier_fee', 'affiliate_subscription', 1, 42);
        $this->ledger->credit('affiliate', $other->id, 500, 'order_profit', 'order', 2);

        $page = $this->earnings->ledgerEntries($mine);

        $this->assertCount(2, $page->items());
        $this->assertSame('affiliate_tier_fee', $page->items()[0]['type']);
    ...
    public function test_dashboard_stats_aggregate_only_this_affiliates_paid_non_test_orders(): void
    {
        $mine = $this->affiliate('Mine');
        $other = $this->affiliate('Other');

        // counts: paid, not test, has paid_at, this month
        Order::factory()->forAffiliate($mine)->create(['final_amount' => 2000, 'paid_at' => now()]);
        Order::factory()->forAffiliate($mine)->create(['final_amount' => 3000, 'paid_at' => now()]);
        // excluded: pending
        Order::factory()->forAffiliate($mine)->pending()->create(['final_amount' => 9999, 'paid_at' => null]);
        // excluded: test order
        Order::factory()->forAffiliate($mine)->create(['final_amount' => 9999, 'is_test' => true, 'paid_at' => now()]);
        // excluded: another affiliate
        Order::factory()->forAffiliate($other)->create(['final_amount' => 5000, 'paid_at' => now()]);
    ...
    public function test_dashboard_stats_today_bucket_excludes_older_paid_orders(): void
    {
        ...
        Order::factory()->forAffiliate($mine)->create(['final_amount' => 1000, 'paid_at' => now()]);
        Order::factory()->forAffiliate($mine)->create(['final_amount' => 4000, 'paid_at' => now()->subDays(3)]);
    ...
    public function test_tier_fee_history_returns_only_fee_debits_for_this_affiliate(): void
    {
        $mine = $this->affiliate('Mine');
        $other = $this->affiliate('Other');

        $this->ledger->credit('affiliate', $mine->id, 8000, 'order_profit', 'order', 1);
        $this->ledger->credit('affiliate', $mine->id, -4900, 'affiliate_tier_fee', 'affiliate_subscription', 1);
        $this->ledger->credit('affiliate', $mine->id, -4900, 'affiliate_tier_fee', 'affiliate_subscription', 1);
        $this->ledger->credit('affiliate', $other->id, -1000, 'affiliate_tier_fee', 'affiliate_subscription', 2);
    ...
    public function test_paid_status_is_matched_by_value_not_enum_instance(): void
    {
        ...
        $mine = $this->affiliate('Mine');
        Order::factory()->forAffiliate($mine)->create([
