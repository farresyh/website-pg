<?php

namespace Tests\Concurrency;

use App\Models\LedgerEntry;
use App\Models\Reseller;
use App\Models\ResellerMembershipTier;
use App\Models\ResellerSubscription;
use App\Services\Ledger\LedgerService;
use App\Services\Reseller\ResellerSubscriptionStatus;
use Illuminate\Foundation\Testing\DatabaseMigrations;
use Illuminate\Support\Facades\Process;
use Tests\TestCase;

/**
 * ADR-056 (grilled 2026-08-30) decision 6 / consequence note: proves the
 * reseller tier-fee charge is race-safe. A double-cron, or a manual admin
 * charge racing the scheduled one, must never debit the fee twice from a
 * reseller's earnings. Two genuinely separate PHP processes each call
 * ResellerTierFeeService::chargeCycle() on the same subscription with
 * earnings funded for exactly ONE fee — only one debit may land, the
 * other must fall through to grace with no ledger write.
 *
 * Requires: docker compose up -d (backend/docker-compose.yml)
 * Run with: php artisan test -c phpunit.concurrency.xml
 */
class ResellerTierFeeConcurrencyTest extends TestCase
{
    use DatabaseMigrations;

    public function test_two_simultaneous_tier_fee_charges_debit_the_fee_only_once(): void
    {
        $ledger = app(LedgerService::class);

        $reseller = Reseller::query()->create([
            'business_name' => 'Concurrency Reseller',
            'markup_pct' => 10,
            'status' => 'active',
        ]);

        $tier = ResellerMembershipTier::query()->create([
            'name' => 'Silver',
            'monthly_fee_sen' => 5000,
            'markup_percent' => 5.0,
            'is_active' => true,
            'sort_order' => 1,
        ]);

        $subscription = ResellerSubscription::query()->create([
            'reseller_id' => $reseller->id,
            'reseller_membership_tier_id' => $tier->id,
            'status' => ResellerSubscriptionStatus::Active,
            'current_period_started_at' => now()->subDays(30),
            'next_charge_at' => now()->subMinute(),
        ]);

        // Fund exactly one fee — a second successful debit would overdraw.
        $ledger->openAccount('reseller', $reseller->id);
        $ledger->credit('reseller', $reseller->id, 5000, 'order_profit', 'order', 1);

        $resultFileA = tempnam(sys_get_temp_dir(), 'reseller_fee_');
        $resultFileB = tempnam(sys_get_temp_dir(), 'reseller_fee_');

        $env = [
            'APP_ENV' => 'testing',
            'DB_CONNECTION' => 'mysql',
            'DB_HOST' => '127.0.0.1',
            'DB_PORT' => '3306',
            'DB_DATABASE' => 'kerox',
            'DB_USERNAME' => 'kerox',
            'DB_PASSWORD' => 'kerox',
        ];

        $command = fn (string $resultFile) => [
            PHP_BINARY, 'artisan', 'app:reseller-test-charge-tier-fee', (string) $subscription->id, $resultFile,
        ];

        $processA = Process::path(base_path())->env($env)->start($command($resultFileA));
        $processB = Process::path(base_path())->env($env)->start($command($resultFileB));

        $processA->wait();
        $processB->wait();

        $outcomes = [file_get_contents($resultFileA), file_get_contents($resultFileB)];

        unlink($resultFileA);
        unlink($resultFileB);

        // Exactly one debit landed; balance is zero, not negative.
        $this->assertSame(0, $ledger->balance('reseller', $reseller->id));
        $this->assertSame(
            1,
            LedgerEntry::query()
                ->where('owner_type', 'reseller')
                ->where('owner_id', $reseller->id)
                ->where('type', 'reseller_tier_fee')
                ->count(),
            'Expected exactly one reseller_tier_fee ledger entry, got outcomes: '.json_encode($outcomes),
        );

        // One process saw active (its debit landed), the other saw grace.
        sort($outcomes);
        $this->assertSame(['active', 'grace'], $outcomes, 'Unexpected outcome pair: '.json_encode($outcomes));
    }
}
