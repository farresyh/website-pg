<?php

namespace App\Services\Reseller;

use App\Models\Reseller;
use App\Models\ResellerMembershipTier;
use App\Models\ResellerSubscription;
use App\Models\ResellerTierChange;
use Illuminate\Support\Facades\DB;

/**
 * ADR-058 58b: the admin-side lifecycle of a reseller's wholesale-tier
 * subscription — assigning/changing the tier (RES-2 / RES-3) and
 * reactivating a lapsed one. The billing side (charging the monthly fee
 * from earnings, active → grace → lapsed) stays in
 * ResellerTierFeeService (ADR-056); this service only manages which tier
 * a reseller is on and writes the `reseller_tier_changes` audit trail.
 */
final class ResellerSubscriptionService
{
    /**
     * Assign a tier to a reseller, or change the tier they're already on.
     * Writes one `reseller_tier_changes` row per real change (never a
     * no-op audit row). A first assignment starts the subscription
     * `active` with a fresh 30-day cycle; a change to an existing
     * subscription swaps the tier only and leaves the billing cycle /
     * status untouched (a mid-cycle tier change doesn't re-bill, and
     * doesn't itself un-lapse a lapsed reseller — that's `reactivate()`).
     */
    public function assignTier(
        Reseller $reseller,
        ResellerMembershipTier $tier,
        ?int $adminUserId = null,
        ?string $note = null,
    ): ResellerSubscription {
        return DB::transaction(function () use ($reseller, $tier, $adminUserId, $note) {
            $subscription = ResellerSubscription::query()
                ->where('reseller_id', $reseller->id)
                ->lockForUpdate()
                ->first();

            $fromTierId = $subscription?->reseller_membership_tier_id;

            if ($fromTierId === $tier->id) {
                return $subscription;
            }

            if ($subscription === null) {
                $subscription = ResellerSubscription::query()->create([
                    'reseller_id' => $reseller->id,
                    'reseller_membership_tier_id' => $tier->id,
                    'status' => ResellerSubscriptionStatus::Active,
                    'current_period_started_at' => now(),
                    'next_charge_at' => now()->addDays(ResellerTierFeeService::CYCLE_DAYS),
                    'grace_until' => null,
                ]);
            } else {
                $subscription->update(['reseller_membership_tier_id' => $tier->id]);
            }

            ResellerTierChange::query()->create([
                'reseller_id' => $reseller->id,
                'admin_user_id' => $adminUserId,
                'from_tier_id' => $fromTierId,
                'to_tier_id' => $tier->id,
                'note' => $note,
            ]);

            return $subscription->refresh();
        });
    }

    /**
     * Bring a `grace` or `lapsed` subscription back to `active` with a
     * fresh 30-day cycle (ADR-056 decision 3/6 — reactivation is an
     * explicit admin action, never an automatic retry). Does not charge
     * the fee; the admin can trigger a charge separately if they want the
     * cycle collected immediately.
     */
    public function reactivate(ResellerSubscription $subscription): ResellerSubscription
    {
        $subscription->update([
            'status' => ResellerSubscriptionStatus::Active,
            'current_period_started_at' => now(),
            'next_charge_at' => now()->addDays(ResellerTierFeeService::CYCLE_DAYS),
            'grace_until' => null,
        ]);

        return $subscription->refresh();
    }
}
