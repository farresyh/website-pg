 ...
 * every reseller-portal money read. `ledger_entries` has no `reseller_id`
 * column — it keys tenant off polymorphic `owner_type` / `owner_id`
 * ('reseller', <resellers.id>), so `BelongsToReseller` / `ResellerScope`
 * cannot protect it. Every read here filters those two columns
 * explicitly, against the `Reseller` passed in — never raw Eloquent on
 * `LedgerEntry` elsewhere, never `DB::table('ledger_entries')` in a
 * reseller-guard path.
 *
 * The reseller runs a SINGLE earnings account (ADR-056 decision 7 — no
 ...
 * `reseller_tier_fee` debits.
 */
final class ResellerEarningsService
{
    ...
    {
        return $this->ledger->balance(LedgerOwnerType::Reseller, $reseller->id);
    }

    /**
     * The reseller's own ledger history, newest first, shaped for the
    ...
    /**
     * The `reseller_tier_fee` debits for this reseller, newest first —
     ...
     */
    public function tierFeeHistory(Reseller $reseller, int $limit = 24): array
    {
        return LedgerEntry::query()
            ->where('owner_type', LedgerOwnerType::Reseller->value)
            ->where('owner_id', $reseller->id)
     ...
     */
    public function dashboardStats(Reseller $reseller): array
    {
        ...
        return [
            'earnings_balance' => $this->balance($reseller),
            'today' => $this->salesAggregate($reseller, $todayStartUtc),
            'this_month' => $this->salesAggregate($reseller, $monthStartUtc),
     ...
     */
    private function salesAggregate(Reseller $reseller, CarbonImmutable $fromUtc): array
    {
        $row = Order::query()
