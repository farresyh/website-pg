 ...
 * change shows within seconds instead of waiting out the 30s TTL.
 * Dispatched by `App\Services\Cache\NextRevalidation::purge()`, which is
 * called next to every backend `forgetCache()`.
 *
 * `ShouldBeUnique` (5s window, ADR-077 PR-3 decision 7) collapses a burst
 * of `forget*Cache()` calls — a price sync touching hundreds of rows, a
 * bulk edit — into a single HTTP POST. Purging the whole `catalog` tag
 * once is correct regardless of how many rows changed; the storefront
 * edge just re-fetches on the next request.
 *
 * Best-effort, like `SendMembershipReceiptJob`: if the storefront is
 * unreachable the mutation already succeeded, and the 30s TTL is the
...
final class PurgeNextCatalogCache implements ShouldBeUnique, ShouldQueue
{
    ...
    {
        // ADR-077 PR-3 decision 7 — its own `revalidation` queue +
        // supervisor so a Price Sync run on `default`/`price-sync` can
        // never head-of-line-block a storefront cache purge. onQueue(),
        // not a redeclared $queue property — Queueable already declares
        // that property (see FulfillOrderJob for the same reasoning).
    ...
    {
        Log::warning(
            'PurgeNextCatalogCache permanently failed — the storefront will still refresh on its 30s Data-Cache TTL',
