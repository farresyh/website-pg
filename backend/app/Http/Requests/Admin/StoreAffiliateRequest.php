...
/**
 * ADR-058 58b (RES-2): a super_admin registers a new affiliate. Route
 * middleware (admin.role:super_admin) is the gate — authorize() stays
 * true. Creates the Affiliate row, its first affiliate_user (triggers the
...
class StoreAffiliateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            // enable consumer Membership; third-party affiliates never can
            ...
            'tier_id' => ['nullable', 'integer', Rule::exists('affiliate_membership_tiers', 'id')->whereNull('deleted_at')],
            'user_name' => ['required', 'string', 'max:255'],
            'user_email' => ['required', 'email', 'max:255', Rule::unique('affiliate_users', 'email')],
        ];
    }
}
