...
/**
 * ADR-059 decision 2: the affiliate's own storefront orders — list +
 * detail, read-only. `Order` carries `BelongsToAffiliate` (ADR-057), so
 * `affiliate.context` already constrains every query here to this tenant;
 * there is no `where('affiliate_id', …)` to write and no way to widen it
...
class OrderController extends Controller
{
    ...
    private function shape(Order $order, bool $detail = false): array
    {
        ...
        return $base + [
            ...
            'affiliate_markup_pct' => (float) $order->affiliate_markup_pct,
