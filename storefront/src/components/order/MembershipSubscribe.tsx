 ...
 * lapse instead). Picking a tier reveals the payment-method step, then
 * POST /api/membership/subscribe returns a CHIP checkout URL to redirect
 * to; the server sets the success/failure return URLs back to
 * /membership?checkout=… (handled by MembershipClient).
 */
      ...
      <div className="grid gap-gutter md:grid-cols-2">
        {options.plans.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={`flex flex-col gap-4 rounded-lg border-2 border-ink p-6 neo ${
                isSelected ? "bg-primary-fixed" : "bg-surface-container-lowest"
              }`}
            >
              <div className="flex items-baseline justify-between gap-3">
                <span className="font-display text-headline-md">{plan.name}</span>
              ...
              <div>
                <p className="font-mono text-price font-bold">
                  ...
                  <span className="font-display text-sm font-bold text-on-surface-variant"> /month</span>
                </p>
              ...
              {isDowngrade ? (
                ...
                >
                  {isSelected ? (
                    <>
                      <CheckCircle size={16} weight="fill" /> Selected
                    </>
                  ) : (
