...
import {
  addResellerUser,
  assignResellerTier,
  chargeResellerTierFee,
  impersonateReseller,
  reactivateResellerSubscription,
  resendResellerInvite,
  type ResellerDetail,
...
interface Props {
  ...
  tiers: ResellerTier[];
  onChanged: (detail: ResellerDetail) => void;
  onRefresh: () => void;
}

const subStatusSeverity: Record<ResellerSubscriptionStatus, "success" | "warn" | "danger"> = {
  active: "success",
  grace: "warn",
  lapsed: "danger",
};
      ...
      <section className="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
        ...
        {sub ? (
          ...
          <p className="mb-3 text-theme-xs text-gray-500 dark:text-gray-400">
            No subscription — this reseller pays the walk-in wholesale rate (standard selling price).
          </p>
        )}
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <Button
            ...
            onClick={() =>
              run("assign", async () => {
                const d = await assignResellerTier(token, r.id, Number(tierId), tierNote || null);
                setTierNote("");
                onChanged(d);
        ...
        {sub && (
          <div className="mt-3 flex flex-wrap gap-2">
            <Button
              ...
              onClick={() =>
                run("charge", async () => {
                  const d = await chargeResellerTierFee(token, r.id);
                  onChanged(d);
              ...
              <Button
                ...
                onClick={() =>
                  run("reactivate", async () => {
                    const d = await reactivateResellerSubscription(token, r.id);
                    onChanged(d);
      ...
      <section className="rounded-lg border border-gray-200 p-4 dark:border-gray-800">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      run(`resend-${u.id}`, async () => {
                        await resendResellerInvite(token, r.id, u.id);
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <Button
            ...
            onClick={() =>
              run("add-user", async () => {
                const d = await addResellerUser(token, r.id, { name: newUserName, email: newUserEmail });
                setNewUserName("");
                setNewUserEmail("");
                onChanged(d);
...
export default function ResellerDetailModal({ isOpen, onClose, token, detail, tiers, onChanged, onRefresh }: Props) {
  return (
      ...
      <DialogPortal>
        ...
        <DialogPositioner>
          <DialogPopup className="w-full max-w-3xl">
            <DialogHeader>
              <DialogTitle>{detail.reseller.business_name}</DialogTitle>
              <DialogHeaderActions>
