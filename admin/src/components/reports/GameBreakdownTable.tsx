...
import type { ReportGameRow } from "@/lib/reports";
import { formatRm } from "./format";

export function GameBreakdownTable({ rows }: { rows: ReportGameRow[] }) {
  if (rows.length === 0) {
      ...
      <table className="w-full text-theme-sm">
        <thead>
          <tr className="border-b border-gray-200 text-left text-theme-xs text-gray-500 dark:border-gray-800 dark:text-gray-400">
            <th className="whitespace-nowrap px-4 py-3 font-medium">Game</th>
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.game_id ?? "unknown"} className="border-b border-gray-100 last:border-0 dark:border-gray-800/60">
              <td className="whitespace-nowrap px-4 py-3 font-medium text-gray-800 dark:text-white/90">{row.game_name}</td>
